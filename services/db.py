...
async def fetch_base_question(topic: str):
    ...
    try:
        ...
        # Then find a random question for this topic
        topic_id = str(topic_doc.get("_id", ""))
        logger.error(f"Searching for questions with topic_id: {topic_id}")
        questions = await db.interview_questions.find({"topic_id": topic_id}).to_list(length=None)
        ...
        logger.error(f"Selected question: {question}")
